...
\newcommand{\paperversion}{Version 1.0.6}
\newcommand{\paperdate}{September 4, 2026}
...
index, and satisfy $\Tur(s)\ge 1$ for every $0\le s\le D$.  Equivalently, these
...
computation in integer arithmetic for the remaining finite cases.  The
source programs and deterministic replay logs are included in the
accompanying repository artifact.
\vskip5pt
...
other half.  In particular, every adjacent minor in this orientation is a
...
Theorem~\ref{thm:offsets}) and fixed-$Q$ strips
...
used to prove the Main Theorem; the proof programs use no floating-point
arithmetic.  In
...
\texttt{certificates/\allowbreak INDEPENDENT\_\allowbreak REPLAY\_\allowbreak SIGNOFF.md}
provides a form for external replay attestation.
...
Development source is maintained in the
\href{https://github.com/skypher/kraw}{project repository}.
This manuscript is Version~1.0.6.  Its source, verification programs, and
deterministic outputs are identified byte-for-byte by
\texttt{certificates/\allowbreak MANIFEST.\allowbreak sha256} in the repository.
Versioned artifacts are distributed from its
\href{https://github.com/skypher/kraw/releases}{releases page}; GitHub serves
as a distribution mirror rather than an immutable archive.  A release includes the source
archive, manuscript PDF, hash records, and replay profiles.  No external
human replay attestation is claimed for this version.  The accepted journal
snapshot will be deposited in a DOI-bearing archive, and the bibliographic
...
\texttt{paper/README.md}.  The command \texttt{make toolchain-info} prints the
Python, SymPy, compiler, Boost, GMP, pdf\TeX, and platform versions.
The targets \texttt{payload-check} and \texttt{verify} check stored hashes
only; they do not execute a proof program.  The target \texttt{audit-fast}
runs the short exact replays, the standalone checker, both mutation suites,
and compiled GMP and Pascal scans through $D=120$.

The target \texttt{replay-all} additionally runs the two longer symbolic
certificates and both exhaustive scans; \texttt{audit-independent} runs the
standalone witness checker, the full Pascal scan, and all twelve mutation
tests.  The command \texttt{make replay-profile} measures the complete replay
under \texttt{/usr/bin/time -v}.  Resource values are machine-dependent and
are therefore excluded from the deterministic proof logs.
...
Szeg\H{o} \cite{szego48}, answering a question of P.~Tur\'an, proved for
...
Most directly, Theorem~5 and equation~(17) of \cite{krasikov-turan} treat
binary Krawtchouk polynomials in a Laguerre--P\'olya generating-function
framework and give a fourth-degree refinement coupling two adjacent Tur\'an
...
not imply Theorem~\ref{thm:main}.  It is the closest explicit antecedent
within the Krawtchouk literature, while differing in both normalization and
conclusion.
...
Ayyer and Kumari \cite[Theorem~2.5]{ayyerkumari22} factor characters of
general linear groups under balanced root-of-unity specializations.  Karmakar
