...
\newcommand{\paperversion}{Version 1.0.5}
\newcommand{\paperdate}{August 26, 2026}
...
\hypersetup{
  ...
  pdftitle={Monotonicity of Turan determinants for binary Krawtchouk polynomials},
  pdfauthor={Leslie P. Polzer},
  pdfkeywords={Krawtchouk polynomials, Turan determinant, unimodality}
}
...
\title{Monotonicity of
Tur\'an determinants for binary Krawtchouk polynomials}
...
\begin{abstract}
Let $M,Q\ge 0$ be integers, $D=M+Q$, and let
$p_s=\coeff{w^s}(1+w)^M(1-w)^Q$, equivalently the binary Krawtchouk value
$K_s(Q;D)$ in its generating-function normalization.  We prove
\[
  p_s^2-p_{s-1}p_{s+1}\ \ge\
  p_{s+1}^2-p_sp_{s+2}
  \qquad (\lceil D/2\rceil\le s\le D).
\]
Thus the Tur\'an determinants are symmetric and weakly unimodal in the coefficient
index, and they are at least $1$ at every index.  Equivalently, these
determinants are the rectangular Schur values
$s_{(2^s)}(1^M,(-1)^Q)$, so the theorem gives positivity and unimodality
for a family of $\mathrm{GL}_D$-character values at involutions.  The proof
...
exact-integer computation for the remaining finite cases.  The
certificate-generating programs and exact replay logs accompany the
manuscript.
\vskip5pt
...
Two immediate corollaries, both obtained by telescoping from the top end
...
$P_n(x)^2-P_{n-1}(x)P_{n+1}(x)$ for a fixed degree window and an argument
...
integer restriction is essential: the corresponding real relaxation need
not be nonnegative.  More precisely, if the integer argument $Q$ is replaced
by a real $q\in[0,D]$ and the coefficients are defined by the same formal
generating series
\[
 [w^s](1+w)^{D-q}(1-w)^q,
\]
then at $(D,q,s)=(4,\frac87,2)$ one obtains
$T_2=-1430/117649<0$.  Section~\ref{sec:related} gives a more detailed
...
\begin{equation}\label{eq:schur-character}
 \Tur(s)=s_{(2^s)}(z).
\end{equation}
When $D\ge1$, this is the irreducible polynomial-character value
\[
...
$(2^s)$ is $(s,s)$, the dual Jacobi--Trudi identity gives
\[
 s_{(2^s)}(z)
...
linear group; see \cite[Chapter~I, Section~3]{macdonald95}.  Finally, for a
...
height $s$ varies.  The character interpretation alone does not imply the
sign: the involution has negative eigenvalues, so ordinary Schur positivity
does not apply to this specialization.  It does, however, identify a
...
\subsection{Why this minor sequence is natural}

For a fixed integer argument $Q$, the values $(p_s)_{s=0}^D$ form a row of
the binary Krawtchouk matrix.  The quantity
\[
 \Tur(s)=
 \det\begin{pmatrix}p_s&p_{s+1}\\p_{s-1}&p_s\end{pmatrix}
\]
is its adjacent $2\times2$ Toeplitz minor.  The theorem therefore orders all
such adjacent minors along half of a Krawtchouk row; reflection orders the
other half.  In particular, every one of these signed-row minors is a
positive integer at least $1$, even though the row itself
oscillates and may contain zeros.  Corollary~\ref{cor:lattice} gives an
equivalent arithmetic interpretation: the minors are the indices of the
lattices generated by successive coefficient vectors.  This makes the
statement stronger than a pointwise Tur\'an sign inequality and places it
naturally beside minor and kernel inequalities used in Hamming-scheme
spectral analysis.  The exact
generating function, standard recurrences, and normalizations for
Krawtchouk polynomials may be found in
\cite[Section~9.11]{koekoek-lesky-swarttouw}.

The endpoint pairs are unimodular: $\Tur(0)=\Tur(D)=1$.  The Main Theorem
therefore gives an exact endpoint minimum and a global ordering for every
row, not just nonvanishing of its adjacent minors: the covolume is smallest,
and equal to one, at both endpoints and never decreases while moving toward
the center.
We do not claim that this observation alone improves a coding bound; its
role here is to identify the arithmetic content of the ordered-minor
statement and the normalization in which the sharp endpoint value is
meaningful.
...
\]
Thus the coefficients oscillate, while the adjacent minors remain positive,
...
Comparisons with normalized Tur\'an inequalities in
Section~\ref{sec:related} are therefore comparisons of phenomena, not claims
of invariance under a change to the present normalization.
...
\subsection{Proof status and computer assistance}\label{subsec:proof-status}

Three kinds of argument occur below.  First, the bulk, center, flow, and
even-minimum regions have proofs in the text; their accompanying programs
replay algebraic identities and finite diagnostics but do not replace the
written implications.  Second, the finite-offset families, the fixed-$Q$
families, and one polynomial-sign lemma in the odd-minimum region are
...
an exhaustive exact-integer computation over a declared finite set.

Every proof decision in the programs uses integer or rational arithmetic.
The primary generators use polynomial factorization, gcd, resultants, and
Sturm root counting in SymPy, together with GMP integer arithmetic.  A second
C++ implementation checks the decisive exported polynomial witnesses without
SymPy or GMP, and repeats the finite scan with Boost integers and a different
coefficient construction.  Hash files identify the reviewed source and
output snapshot but do not themselves prove that a program was run.
Section~\ref{sec:machine} gives the certificate rules, trust boundary, and
...
top initial values are $p_D=(-1)^Q$ and $p_{D-1}=(-1)^Qx$.  The sequence obeys the step-$1$
recurrence (contiguous in $s$; from the log-derivative of the generating
function):
...
The \emph{sign} of the discriminant is basis-invariant: re-expressing $T_s$
in a shifted basis multiplies it by a positive square.  For example, the
$(p_{s-2},p_{s-1})$ discriminant is the displayed one times
\[
...
\[
 (D+2)^2-x^2\ge (D+2)^2-D^2=4D+4>0.
\]
\end{proof}

The rank-one central relation $(D+2)p_{c+1}=xp_c$ supplies the initial
condition for the two recurrence arguments in
Sections~\ref{sec:evenflow} and~\ref{sec:oddmin}.
...
\subsection{The tail: Riccati comparison flow}

\begin{theorem}[tail flow]\label{thm:flow}
...
\]
for $0\le x\le D$ (linear decreasing in $x^2$, with value $7D^2+16D$ at
...
\begin{tikzpicture}[scale=0.92]
  ...
  \node at (5.35,5.6) {\small flow};
  \node at (5.35,5.22) {\small (Thm~\ref{thm:flow})};
...
The bulk and flow regions are separated by the displayed circle.  The
...
Theorems~\ref{thm:bulk}--\ref{thm:offsets}
and Propositions~\ref{prop:collapse}--\ref{prop:qle2} cover every cell with
...
finite-offset nondegeneracy remainder $D<1191$; the fixed-$Q$ edge-flow
...
gap, $Q\le 2$ \ ($Q=\min$) & Proposition~\ref{prop:qle2} \\
...
\end{tabular}
\medskip
\caption{Coverage of the proof.}
\label{tab:coverage}
\end{table}

\section{Reflection collapse and finite families}\label{sec:collapse}

The finite part of the gap is treated in two ways: by fixed-offset families
(Proposition~\ref{prop:collapse} and Theorem~\ref{thm:offsets}) and by
fixed-$Q$ strips (Proposition~\ref{prop:qle2} and Theorem~\ref{thm:strips}).
...
\begin{proof}
For $s=(D+u)/2$ reflection gives $p_{s-u}=(-1)^Qp_s$; feeding this single
constraint through the $(u+2)$-step recurrence chain determines the ratio
...
\end{equation}
at every cell satisfying the constraint, without division.  Hence a cell is
proved as soon as
$G\ge 0$ there and $(\lambda,\mu)\ne(0,0)$.  The
...
\item \emph{(sector)}  $\mathrm{gap}(u)$ with $Q\ge 3$ forces
  ...
  $2Q+3<(D+3)/U+U/(D+3)$ with $(D+3)/U>8$ (indeed $U<U_-\le D+3$
  makes $\tau=(D+3)/U>1$, and then $\tau+1/\tau>2Q+3\ge9$ forces
  $\tau>(9+\sqrt{77})/2>8$), and $M\ge 425$: the exact
  census already quoted in Section~\ref{sec:regimes} (and repeated by
  ...
  only $M\ge425$.
\item \emph{(two reduced constraints)}  For the primary pinning let
  $g_1=\gcd(\lambda_1,\mu_1)$ and strip
...
\item \emph{(nondegeneracy)}  For \emph{each} of the two pinnings, the
  ...
  $M\le1145$ implies $D=M+Q<1191<1200$.  Where the first common factor
  $g_1$ vanishes, the alternative pinning
  $p_{s+1}=\eps\,p_{k_0-1}$ takes over: a further resultant check proves
  $\{g_1=0\}\cap\{g_2=0\}$ contains no integer sector point with $M>1145$,
  so there the second common factor is nonzero, the second reduced
  identities are valid, and the second reduced pair is nonzero by its own
  nondegeneracy check above.  (Nonvanishing of $g_2$ alone would not
  suffice: the conclusion also needs
  $(\lambda'_2,\mu'_2)\ne(0,0)$ at the cell.)
\end{enumerate}
...
To make the computational mechanism inspectable without running the
generator, consider $u=4$ and $\eps=+1$.  After removing the common factor
...
\subsection{Per-\texorpdfstring{$Q$}{Q} closed forms: \texorpdfstring{$\min(M,Q)\le 2$}{min(M,Q) <= 2}}
...
(Equivalently, the unreoriented symbolic denominator has the single negative
...
\subsection{Per-\texorpdfstring{$Q$}{Q} strip checks: \texorpdfstring{$3\le Q\le 12$}{3 <= Q <= 12}}
...
\begin{theorem}[per-$Q$ strip checks]\label{thm:strips}
After relabeling so that $Q=\min(M,Q)$, the Main Theorem holds for every
...
\]
For $Q=3,\ldots,12$, the exact respective onsets are
\[
...
\section{Even-minimum gap cells}\label{sec:evenflow}

\begin{theorem}[even-minimum gap cells]\label{thm:evenflow}
After relabeling so that $Q\le M$, every gap cell with $Q$ even and
$u\ge 15$ has $T_s\ge 0$.
\end{theorem}

\begin{proof}
Define ($j$ runs over top-half indices)
\[
...
\]
Call the region $(2j-D)^2<(D+1)^2-x^2$ the \emph{oscillatory zone}.  The
...
\item \emph{($(p,d)$ system)}  First order, polynomial coefficients:
  \[
...
\item \emph{(comparison map)}  $\zeta_j:=-d_j/p_j$ obeys
  ...
  $\kappa=1$ map is exactly tan-addition:
  \[
    F\bigl(\sqrt\Lambda\,\tan a\bigr) \;=\; \sqrt\Lambda\,\tan(a+\theta^*),
    \qquad \tan\theta^* \;=\; \frac{\sqrt\Lambda}{x} .
  \]
  Anchors from the reflection ($Q$ even here, $p_{D-j}=p_j$): for $D$ even,
  ...
  tan-addition majorant then gives
  \[
    \zeta_{c+\ell} \;\le\;
...
\section{Odd-minimum gap cells and proof assembly}\label{sec:oddmin}

\subsection{Odd-minimum gap cells}
...
\begin{proof}
This is the exact polynomial certificate used in the odd-minimum induction.
For transparency, its symbolic obligations are specified here.  Put
\[
\begin{gathered}
 W_1=x(u+1)+g_u,\quad
...
correct unsquared sign and proves the finite-state implication.  Finally,
after multiplication by the positive factor in $O_4$,
$K^2-\disc_2$ is linear decreasing in $X$, with coefficient
...
\begin{theorem}[odd-minimum gap cells]\label{thm:oddmin}
...
In $y$-coordinates the quadratic form
  \eqref{eq:qform} of Theorem~\ref{thm:bulk} reads, exactly,
  $G(y_s)\,p_s^2=(s+2)(D+1-s)(D+2)^2\,T_s$.  Thus, when $p_s\ne0$,
  $T_s\ge 0$ iff $y_s$ avoids the open window
  $(y_-(u),y_+(u))$ of the upward parabola
  \begin{equation}\label{eq:window}
    G(y) \;=\; u(s+2)\,y^2 - x(u+1)(D+2)\,y + (u+2)(D+1-s)(D+2)^2
  \end{equation}
...
The base inequality $y_{c+1}\ge y_+(\text{first window})$ holds: for $D$
  even (first window at $u=2$), eliminating the square root reduces this to
  $x^2\le(D+4)^2$, which always holds; for $D$ odd (first window at $u=1$) it
  is the final assertion of Lemma~\ref{lem:odd-transition}.
...
$(p_j,p_{j+1})\in\mathcal C_{2j-D}$ along the whole strip.  At the target,
...
replays: rational arithmetic (SymPy) for symbolic identities, integer Sturm
...
used to prove the Main Theorem; the proof programs are float-free.  In
particular the orbit tan-bound comparison in the even-minimum verifier is
...
$\sqrt\Lambda$, and tan-addition preserves that form.  The
...
\item \emph{coefficient-positive polynomials}: after explicit integer
  ...
  coefficients nonnegative, so positivity is inspection;
\item \emph{Sturm counts} on explicitly given intervals for the univariate
  cases (per-$Q$ pairing thresholds, interval base case, finite strips);
...
\item \emph{exhaustive exact scans} for the remaining finite regions ($M,Q\le 90$;
  ...
  values are cross-checked against every binomial coefficient
  for $D\le12$ and against deterministic coefficients in six larger rows up
...
\end{itemize}
\noindent
The committed text files are deterministic replay logs, not separate
proof objects: the proof obligations and their exact checks are specified in
the corresponding source programs.  In particular, elapsed wall-clock times
are excluded from the logs.  The manifest hashes bind each log to the source
program declared to have produced it; a hash establishes snapshot integrity,
not that the program was executed.  Acceptance of a computer-assisted step requires
inspection or replay of that program, not merely inspection of a
\texttt{PASS} line.  The finite-offset log additionally records, for every
...
odd-minimum verifier checks the fully cleared transition identity
\eqref{eq:cleared-transition} before any endpoint positivity test.

Thus the human-readable text is logically complete only together with the
certificate-generating sources.  The trusted base consists of the language
runtimes, compiler, and exact-arithmetic libraries listed in
Section~\ref{subsec:availability}.  The primary path uses SymPy's
factorization, gcd, resultant, and Sturm implementations.  The independent
C++ path checks their decisive outputs by separately implemented arithmetic;
it reduces the shared software path but does not remove the compiler, Boost
integer primitives, or the independent checker source from the trust base.
The file
\texttt{certificates/\allowbreak CERTIFICATE\_\allowbreak MAP.md}
maps every
machine-dependent theorem to its generator, exact log, proof obligations,
and replay command.  The companion
\texttt{certificates/\allowbreak REVIEW\_\allowbreak CHECKLIST.md}
...
The additional
\texttt{certificates/\allowbreak INDEPENDENT\_\allowbreak CHECKS.md}
specifies the second implementation and its mutation tests.  Its separate
human replay form remains unsigned until completed by another person.
...
programs.  They reduce the machine-assisted portions to finite lists of
...
\item Let $f(M,Q)$ have total degree $d$, put $t=Q/M$, and write
  \[
    M^{-d}f(M,Mt)=L(t)+\sum_{\nu\ge1}M^{-\nu}L_\nu(t).
  \]
  On $0\le t\le t_0$, suppose a mesh of width $t_0/N$ has minimum
  $m_L$ for $L$, and $|L'|\le B$.  Then
...
The inputs are not opaque expanded expressions: for each
$u\in\{4,\dots,14\}$ and $\eps\in\{\pm1\}$ they are defined recursively by
...
\paragraph{Odd-minimum endpoints.}
...
not hard-coded, by the odd-minimum verifier.

\subsection{Availability and trust boundary}\label{subsec:availability}

Development source is maintained at \url{https://github.com/skypher/kraw}.
The exact submission snapshot is \texttt{v1.0.5}, distributed from
\url{https://github.com/skypher/kraw/releases/tag/v1.0.5}.  This version adds
the independent verification layer described below.  The tagged snapshot
carries
\texttt{certificates/\allowbreak MANIFEST.sha256}, which binds the manuscript,
programs,
and deterministic outputs byte-for-byte; no claim is made that an untagged
development branch reproduces that snapshot or that GitHub releases are
immutable.  GitHub serves as a distribution mirror.  Its release assets
include the source archive, manuscript PDF, hash records, and local
independent-assurance profile; the separate human replay record remains
unsigned.  Before publication, the exact accepted snapshot
must also be deposited in a persistent archival repository and its DOI added
to the final bibliographic record.  Because the source
programs are part of the proof, this deposit is an acceptance condition, not
an optional post-publication convenience; an accepted version should not
refer only to a moving development branch.

To avoid a circular identifier (the full manifest includes the manuscript),
\texttt{certificates/\allowbreak PAYLOAD.sha256} separately hashes only the proof
generators, exact logs, dependency locks, license, certificate map, and
referee checklist.  Its own SHA-256 digest is
\begin{center}
\small\nolinkurl{8659c3004354cc7dbf95a8509bb3e4ed7787c352ed4ff69586c3e92f7fcbdbaa}
\end{center}
This digest identifies the proof payload printed with this submission; the full
manifest additionally binds the article and the PDF built in the reference
toolchain.  The
repository contains \texttt{CITATION.cff} and release metadata ready for an
archival deposit.  The archival DOI will be inserted after deposit of the
accepted snapshot.
...
The primary trusted base is Python~3, SymPy~1.12, a C++17 compiler, and GMP.
The independent path uses the same compiler but replaces the decisive SymPy
operations and GMP scan with repository-local polynomial algorithms and
Boost multiprecision integers.  The programs themselves remain subject to
inspection.  They contain no floating-point proof computation: every
acceptance predicate, including the end-to-end orbit checks, is integer or
rational arithmetic.

The command \texttt{make toolchain-info} prints the Python, SymPy, compiler,
Boost, GMP, pdf\TeX, and platform versions of a replay environment.
\texttt{make audit-fast} combines the manifest checks, all five short exact
replays, the standalone independent checker, both mutation suites, and
compiled GMP and Pascal scans through $D=120$.  These commands do not
replace the complete replay, but they expose environment drift and exercise
every short proof mechanism before the longer symbolic stages are attempted.
The command \texttt{make replay-profile} runs the complete replay under
\texttt{/usr/bin/time -v}; the archival release should include the resulting
wall time and peak resident memory together with the toolchain report.
Resource values are machine-dependent and are therefore not
embedded in the deterministic proof logs.

The Make targets \texttt{payload-check} and \texttt{verify} check stored
hashes only; they do not execute a proof program.  The target
\texttt{replay-short} runs the five short exact verifiers, while
\texttt{replay-all} additionally runs the two longer symbolic certificates and
both exhaustive scans.  The target \texttt{audit-independent} runs the
standalone witness checker, the full Pascal scan, and all twelve mutation
tests.  Reference software versions and the individual commands are recorded
in \texttt{paper/README.md}.

\subsection{Verification inventory}

Each row names a program and its committed output in
\texttt{certificates/}; the C++ scanners and independent checker are in
\texttt{cpp/}.
The accompanying \texttt{certificates/\allowbreak MANIFEST.sha256} records the
exact artifact hashes, and \texttt{certificates/\allowbreak README.md} states the
artifact
trust boundary and replay convention.
...
Artifact & Verifies \\
\midrule
\texttt{recurrence-and-small-scan} &
  Recurrence identity ($M,Q\le30$) and $T_s\ge0$ exhaustively for
  $M,Q\le90$. \\
\texttt{exhaustive-scan-D1200} &
  Exhaustive exact scan of $289{,}261{,}901$ cells with zero negatives for
  all $D\le 1200$, including per-step divisibility tests and deterministic
  convolution audits (Proposition~\ref{prop:finite-scan}). \\
\texttt{independent-scan-D1200} &
  The same finite set through an independent Boost/Pascal construction,
  including coefficientwise agreement of both parents, reflection, and
  endpoint checks. \\
\texttt{regime-decomposition} &
  Theorems~\ref{thm:bulk}--\ref{thm:flow}, Corollary~\ref{cor:coverage},
  Proposition~\ref{prop:collapse} (symbolic identities, grids, end-to-end
  coverage $D\le 90$, gap census). \\
\texttt{finite-gap-offsets} &
  Theorem~\ref{thm:offsets}: offsets $u=4,\dots,14$, both parities;
  sector estimate; threshold $D=4586$. \\
\texttt{small-argument-cases} &
  Proposition~\ref{prop:qle2}: $Q\le 2$ for all $D$. \\
\texttt{fixed-argument-strips} &
  Theorem~\ref{thm:strips} and Corollary~\ref{cor:finite-coverage}:
  per-$Q$ strips $Q=3,\dots,12$; threshold census $D=14826$. \\
\texttt{even-minimum-gap} &
  Theorem~\ref{thm:evenflow}: even-minimum gap cells, all $D$ (symbolic
  identities, angular bound, end-to-end checks for both $D$ parities). \\
\texttt{odd-minimum-gap} &
  Theorem~\ref{thm:oddmin}: odd-minimum gap cells (symbolic interval
  algebra, endpoint and finite-strip coefficient positivity,
  end-to-end recurrence checks near $D=14828$). \\
\texttt{independent-certificate-check} &
  Separate exact replay of the fixed-argument ratios and Sturm chains,
  finite-offset factors, sector bounds and resultants, and odd-minimum
  endpoint signs; all twelve injected mutations are rejected. \\
\bottomrule
\end{tabular}
\medskip
...
The classical Tur\'an inequality (Szeg\H{o} \cite{szego48}, for Legendre
polynomials, answering P.~Tur\'an) asserts
...
determinants as functions of the argument $x$, with the degree window fixed,
for broad classes of orthogonal polynomials (for general background see
\cite{szego-book}).  Five strands are closest to the present theorem.
...
inequality.  Bustoz and Ismail \cite[Section~2, especially (2.4)]{bustozismail97}
...
determinant indices under recurrence hypotheses.  These results control the
\emph{sign} or propagated sign of $\Tur$, not the ordered comparison
$\Tur(s)\ge\Tur(s+1)$ asserted here.
The present integer arguments also belong to the Krawtchouk orthogonality
support, so the location of the argument is not the distinction.  Moreover,
the cited general criteria are normally formulated after a prescribed
orthogonal normalization.  As explained in
Section~\ref{subsec:normalization}, degree-dependent rescaling changes the
present monotonicity question; no deduction between the normalized and
coefficient-normalized statements is being asserted.
...
\cite{krasikov-laguerre,krasikov-turan,krasikovlitsyn96,krasikov-cd}.
Most directly, \cite[Theorem~5 and (17)]{krasikov-turan} includes binary
Krawtchouk polynomials in a Laguerre--P\'olya generating-function framework
and gives a fourth-degree refinement coupling two adjacent Tur\'an
determinants.  That framework is written for exponential coefficients
...
Moreover, its conclusion controls positivity and a product of adjacent
determinants, not the ordered difference
$\Tur(s)-\Tur(s+1)$ in the present coefficient normalization.  Thus it does
not imply Theorem~\ref{thm:main}, but it is the closest explicit antecedent
within the Krawtchouk literature and should not be conflated merely with an
envelope estimate.  The proof
here instead uses the exact arguments of
Sections~\ref{sec:evenflow}--\ref{sec:oddmin}.

\emph{Specialized Schur functions and character values.}
Proposition~\ref{prop:schur} identifies $\Tur(s)$ with the rectangular
character value at the order-two element $\operatorname{diag}(I_M,-I_Q)$.
Ayyer and Kumari \cite[Theorem~2.5]{ayyerkumari22} factor general-linear
characters under balanced root-of-unity specializations.  Karmakar
\cite[Theorem~4.1]{karmakar25} gives formulas at arbitrary order-two
conjugacy classes, including the present involutions; outside the factorized
cases these formulas are alternating sums of products of dimensions.  They
compute individual character values but do not compare the rectangles
$(2^s)$ and $(2^{s+1})$, nor do they imply their positivity in the present
specialization.  Reiner and Stanton \cite{reinerstanton98} prove symmetry and
...
sequence here, so neither result implies the other.  These works nevertheless
place the Main Theorem alongside character specialization and Schur
unimodality, rather than only classical Tur\'an inequalities.
...
available \cite{liwong00}.  Neither line of work treats monotonicity of the
degree-index Tur\'an determinant at a fixed Krawtchouk argument.
...
linear-programming bounds.  We are not aware of the specific quantity
$\Tur(s)=p_s^2-p_{s-1}p_{s+1}$ (in the coefficient/degree index, at a fixed
integer argument) having been studied there.

We did not find the monotonicity statement of Theorem~\ref{thm:main} in the
orthogonal-polynomial, coding-theoretic, or specialized-character literature.
Our novelty claim is limited to that precise statement for the coefficient
normalization of the two-parameter family $(1+w)^M(1-w)^Q$ at integer
$(M,Q)$, equivalently the ordered rectangular character values in
\eqref{eq:schur-character}.

\section{Further questions}\label{sec:open}

\begin{enumerate}
\item \textbf{Uniform-$Q$ pairing structure.}  The per-$Q$ strip
  ...
  with positive neighbors two degrees below.  A contiguity recursion
  in $Q$ ($p_s^{(Q+1)}=p_s^{(Q)}-p_{s-1}^{(Q)}$) that provably preserves
  the paired cone would treat the entire gap uniformly and give a second,
  asymptotics-free proof of the Main Theorem.
\item \textbf{Beyond the symmetric weight.}  Does Tur\'an-step monotonicity
  persist for the nonsymmetric two-root coefficient family
  $p_s(a,b)=\coeff{w^s}(1+aw)^M(1-bw)^Q$, with $a,b>0$ and $a\ne b$, or for
  other products of two
  real-rooted polynomials whose roots interlace across $0$?  The reflection
  symmetry used for the central initial values and the finite recurrence
  identities is special to $a=b$; which parts of the argument survive its
  loss?
\item \textbf{Combinatorial meaning of $\Tur(s)\ge 1$.}  On the top half
  the Tur\'an determinant is not just nonnegative but $\ge 1$ (integer
  telescoping).  Is there a lattice-path or coding-theoretic injection
...
(\texttt{gpt-5.6-sol}) at max reasoning effort.  The system was used as an
