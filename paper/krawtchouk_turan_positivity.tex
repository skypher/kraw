...
\newcommand{\paperversion}{Version 1.0.7}
...
A small row illustrates the statement.  For $(M,Q)=(5,2)$,
\[
...
In particular, \cite[Theorem~5 and (17)]{krasikov-turan} couples adjacent
Tur\'an determinants in an exponential-coefficient normalization.
...
\]
is its adjacent $2\times2$ Toeplitz minor.  Corollary~\ref{cor:lattice}
...
comparison in the even-minimum verifier is
...
\href{https://github.com/skypher/kraw/blob/v1.0.6/certificates/REVIEW_CHECKLIST.md}{\texttt{certificates/\allowbreak REVIEW\_\allowbreak CHECKLIST.md}}
...
This manuscript is Version~1.0.7.  Its source and PDF are maintained in the
...
The command \texttt{make toolchain-info} prints the
...
$\Tur(s)-\Tur(s+1)$ in the present coefficient normalization.  Its
normalization and conclusion therefore differ from those of
Theorem~\ref{thm:main}.
...
\cite[Theorem~2.5]{ayyerkumari22}, treats general linear characters under
balanced root-of-unity specializations.  Karmakar
...
\item \textbf{Uniform-$Q$ pairing structure.}  The per-$Q$ strip
  ...
  $p_s^{(Q)}=\coeff{w^s}(1+w)^M(1-w)^Q$.  If the contiguity recursion
  $p_s^{(Q+1)}=p_s^{(Q)}-p_{s-1}^{(Q)}$ preserves the paired cone, it would
  treat the entire gap uniformly in $Q$ and yield another proof of the
  Main Theorem.
