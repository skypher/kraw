...
\newcommand{\paperversion}{Version 1.0.9}
...
The \emph{sign} of the negated discriminant is unchanged under the indicated
basis changes: expressing $T_s$ in a shifted recurrence basis multiplies the
negated discriminant by a positive square.  For example, the negated
...
For each top-half index $j$ with $p_j\ne0$, define the scaled ratio
\[
 y_j:=\frac{(D+2)p_{j+1}}{p_j}.
\]
At a gap cell $s$ with $p_s\ne0$, the quadratic-form identity
\eqref{eq:qform} gives
\[
 G(y_s)p_s^2=(s+2)(D+1-s)(D+2)^2T_s,
\]
where
\begin{equation}\label{eq:window}
 G(y)=u(s+2)y^2-x(u+1)(D+2)y
       +(u+2)(D+1-s)(D+2)^2.
\end{equation}
Since $u\ge1$, this is an upward parabola, and the gap inequality
\eqref{eq:gap} gives two real roots $y_-(u)<y_+(u)$.  We call the open
interval between them the \emph{gap window}.  Thus $T_s\ge0$ if and only if
$y_s$ avoids this interval.

For odd $Q$, the argument keeps the scaled ratio above the upper endpoint
of the gap window.  The next lemma gives the endpoint formula, obtained by
the quadratic formula with $s=(D+r)/2$ in \eqref{eq:window}, and the comparison used to
propagate this bound under the recurrence.  The projective formulation in
the proof of Theorem~\ref{thm:oddmin} also includes zero coefficients.
...
For a top-half index $j$, write $r=2j-D$.
...
By the window interpretation in \eqref{eq:window}, it is enough to prove
...
either $p_s=0$ and \eqref{eq:qform} gives
$T_s=u\,p_{s+1}^2/(D+1-s)\ge0$, or
...
This manuscript is Version~1.0.9.  Its source and PDF are maintained in the
